...
source(here("R", "plot-cases-100k.R"))
...
cases <- filter(sgtf$posterior, value_type == "cases" &
                  variant_relationship == "correlated") %>%
  select(date, region, type, median, q5, q95)

# get VOC fracton from correlated model
voc_frac <- filter(sgtf$posterior, value_type == "voc_frac" &
                     variant_relationship == "correlated") %>%
  select(date, region, type, median, q5, q95)

# set up dates
forecast_start <- max(sgtf$data$date)
forecast_end <- max(cases$date)
```
...
```{r data, fig.width = 9, fig.height = 9, fig.cap = "Daily cases in England and by UKHSA region, with S-gene target result (failed, confirmed detected, or unknown), and centred 7-day moving average up to date of data truncation (dotted line)."}
plot_daily_cases(
...
```{r}
plot_inc_100k(cases = cases, forecast_start = forecast_start)
```
