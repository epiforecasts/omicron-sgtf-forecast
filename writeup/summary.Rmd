...
We first visualised our combined data sources (cases by specimen date and SGTF status by specimen date). We then fit both models separately to data for England and to each NHS region. Using these model fits we report posterior estimates from the best fitting model for the following summary statistics of epidemiological interest: growth rates by variant and overall, the time-varying transmission advantage for the Omicron variant, case counts overall and by variant for a 14 day forecast window assuming constant future growth, the date at which Omicron will become dominant in England and in each NHS region, and the estimated cumulative percentage of the population with a reported Omicron case. 

To explore the potential for bias in cases that have a reported SGTF status we also fit the correlated model to case counts and S-gene tested status and reported the apparent transmission advantage for those with a S-gene status over time. Any variation in this metrice from 100% may be interpreted as indicating biased sampling of those with known S-gene status which may bias our main results.
...
* We make no adjustment for the background rate of SGTF observed for Delta or other factors that might lead to SGTF.
...
* Our estimates are at the national or NHS region scale. There may be additional variation within these areas that these estimates cannot capture.
...
* A model that assumed that variants growth rates varied over time relative to each other performed better in all regions than a model where the relative difference was cosntant over time. 
...
* Assuming growth rates stay as currently estimated we forecast that approimately 7.5% to 10% of the English population will have reported an Omicron case by the end of 2021 though significant regional variation.
...
kable(scores, caption = "Estimated differences in the ELPD metric (with
 standard errors) between the scaled and correlated models for England and NHS
...
```{r growth, fig.cap = "Estimates of the time-varying daily growth rate for England and by NHS region (using the log scale approximation) for reported cases overall, with the Omicron variant, and not with the Omicron variant. Growth rates are assumed to be piecewise constant with potential changes every 3 days. Growth rates are assumed to be constant beyond the forecast horizon (vertical dashed line). 90% and 60% credible intervals are shown."}
...
```{r voc-frac-natural, fig.cap = "Estimates of the proportion of reported cases with the Omicron variant for England and by NHS region using both the scaled model and the correlated model on the natural scale. The forecast horizon is indicated using a dashed line. 90% and 60% credible intervals are shown."}
plot(
...
```{r voc-frac-logit, fig.cap = "Estimates of the proportion of reported cases with the Omicron variant for England and by NHS region using both the scaled model and the correlated model on the logit scale. Note that scaled model is effectively assuming a linear relationship on this scale. The forecast horizon is indicated using a dashed line. 90% and 60% credible intervals are shown."}
plot(
  sgtf$posterior, as.data.table(sgtf$data), type = "voc_frac",
...
```{r omicron-95, fig.cap = "Estimated dates at which 95% of reported cases will have the Omicron variant for England and NHS regions. Points represent the median date and ranges represent the 90% credible interval.", fig.height = 6, fig.width = 6}
...
```{r log-cases, fig.cap = "Estimates of the number of reported cases by variant and overall for England and each NHS region on the log 2 scale. Estimates are shown both within the window supported by the data and for 14 days afterwards as a forecast where it is assumed that growth rates for each variant are stable. The forecast horizon is indicated using a dashed line. 90% and 60% credible intervals are shown."}
plot(
...
```{r cumulative-population, fig.cap = "Estimates of the cumulative percentage of the population with a reported Omicron casep in England and by NHS. The forecast horizon is indicated using a dashed line. 90% and 60% credible intervals are shown."}
