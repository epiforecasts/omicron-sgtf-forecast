...
* Assuming growth rates stay as currently estimated we forecast that approimately 7.5% to 10% of the English population will have reported an Omicron case by the end of 2021 though with significant regional variation.
...
```{r voc-frac-natural, fig.cap = "Estimates of the proportion of reported cases with the Omicron variant for England and by NHS region using both the scaled model and the correlated model on the natural scale. The forecast horizon is indicated using a dashed line. 90% and 60% credible intervals are shown. Note that observation model uncertainty is not captured by these credible intervals as so they may appear overly precise compared to the data."}
plot(
...
```{r voc-frac-logit, fig.cap = "Estimates of the proportion of reported cases with the Omicron variant for England and by NHS region using both the scaled model and the correlated model on the logit scale. Note that scaled model is effectively assuming a linear relationship on this scale. The forecast horizon is indicated using a dashed line. 90% and 60% credible intervals are shown. Note that observation model uncertainty is not captured by these credible intervals as so they may appear overly precise compared to the data."}
plot(
  sgtf$posterior, as.data.table(sgtf$data), type = "voc_frac",
