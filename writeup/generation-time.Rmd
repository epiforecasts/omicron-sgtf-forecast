...
We use the same methodology to estimate growth rates as in our earlier report on the time-varying transmission advantage of the Omicron variant.
...
Where $R$ is the effective reproduction number, $\bar{G}$ is the mean of the gamma distributed generation time, $r$ is the daily growth rate, and $k$ is the squared coefficient of variation which can be reparameterised as $\frac{\bar{G}^2_\mathrm{SD}}{\bar{G}^2}$ where $\bar{G}^\mathrm{SD}$ is the standard deviation of the gamma distribution.
...
\begin{align}
 ...
 R^\mathrm{D}_{lt} &= \left(1 + \bar{G}_\mathrm{D}  k^\mathrm{D} r^\mathrm{D}_{lt}  \right)^{\frac{1}{k^\mathrm{D}}} \\
 R^\mathrm{O}_{lt} &= \alpha R^\mathrm{D}_{lt} \\
 r^\mathrm{O}_{lt} &= \frac{(R^\mathrm{O}_{lt})^k^\mathrm{O} - 1}{k^\mathrm{O} \bar{G}_\mathrm{O}} \\
