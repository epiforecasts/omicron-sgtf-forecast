...
\begin{align}
  ...
  \frac{\bar{G}^\mathrm{O}}{\bar{G}^\mathrm{D}} &\sim  \text{LogNormal}\left(0, 0.25 \right) \\
...
```{r syn-scenario-plot, fig.height = 9, fig.width = 9, fig.cap = "Growth rates scenarios based on an observed growth rate for one variant (black line) and the resulting growth rate for another variant varying the assumed transmission advantage (coloured lines), generation time mean (x facet - as a fraction of baseline assumed value of 3.2 days), and generation time standard deviation (y axis, as a fraction of the baseline assumed value of 2.1 days). The central facet represents a scenario where variants share a generation time."}
syn_sims <- fwrite(here("data", "simulations", "uk-scenario-simulations.csv"))
