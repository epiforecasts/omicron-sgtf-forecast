...
#### Estimated impact on the reproduction number associated with plan B

```{r}
plan_b_diff <- fread(here("data", "retrospective", "plan_b_diff.csv"))
plan_b_diff  %>%
  select(-gt_diff) %>%
  mutate(stratification = str_to_sentence(stratification),
         gt_type = str_to_sentence(gt_type)) %>%
  rename(data_stratification = stratification,
         `Generation time prior` = gt_type) %>%
  janitor::clean_names(case = "sentence") %>%
  kable(caption = "Summarised posterior estimates of the additive and
   multiplicative impact associated with plan B on the estimated reproduction
   number of Omicron cases stratified by generation time prior assumption and
   the stratification of the data used to initially estimate growth rates.
   These estimates have been derived by comparing reproduction estimates from
   the 9th of December 2021 with reproduction number estimates from the 17th of
   December.The multiplicative effect is the result of dividing the
   reproduction number estimate from the 17th with that from the 9th"
  ) # nolint
```
## Discussion
