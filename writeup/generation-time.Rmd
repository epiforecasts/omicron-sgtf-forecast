...
\begin{align}
 R^\mathrm{D}_{lt} &= \left(1 + g^\mathrm{D}  k^\mathrm{D}_{lt} r^\mathrm{D}_{lt}  \right)^{\frac{1}{k^\mathrm{D}}} \\
 R^\mathrm{O}_{lt} &= \alpha R^\mathrm{D}_{lt} \\
 g^\mathrm{O}_{lt} &= \frac{(R^\mathrm{O}_{lt})^2 - 1}{k^\mathrm{O} \mu^\mathrm{O}} \\
 \sigma^\mathrm{D}_{lt} &= \sqrt{\sigma^2 + (\tau^\mathrm{O}_{lt})^2} \\
 r^\mathrm{O}_{lt} &\sim \text{Normal}\left(g^\mathrm{O}_{lt}, \sigma_{lt}^\mathrm{O} \right) \\
\end{align}

Here, $l$ indicates location and $t$ time (day), superscripts D and O denote Delta and Omicron, respectively, $g$ and $k$ are the mean and coefficient of variation of the generation time, $r$ is the estimated growth rate, $\tau$ its standard deviation and $R$ the reproduction number. Delta generation time parameters are sampled (with uncertainty) from the Hart et al. estimates, growth rates (with uncertainty) estimated from the case data, and all other parameters estimated or calculated according to the given equations.
