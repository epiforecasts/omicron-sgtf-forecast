...
daily_age <- read_csv(
  ...
  filter(date >= date_data_start) %>%
  filter(date <= date_forecast_start)

# load posterior estimates
sgtf_by_age <- load_results(age_date, type = "sgtf-by-age")

sgtf_by_age_posterior <- sgtf_by_age$posterior[
  variant_relationship == "correlated"
][date <= date_forecast_start]
...
plot_daily_cases(
  ...
  start_date = date_data_start, smooth_total = FALSE
) +
...
plot_daily_cases(
  daily_age, truncate_date = date_forecast_start, caption = "",
  start_date = date_data_start, smooth_total = FALSE, by = "age_group"
) +
  facet_wrap(~ age_group, scales = "free")
```

```{r data-by-age-regions, fig.cap = "Daily cases in England and by UKHSA region, with S-gene target result (failed, confirmed detected, or unknown), and centred 7-day moving average starting from the 23rd of November 2021 and including data up to the 23rd of December 2021 (dotted line). Source: UKHSA and coronavirus.gov.uk; data by specimen date.", fig.width = 16}
plot_daily_cases(
  filter(daily_age, !(region %in% "England")),
  truncate_date = date_forecast_start, caption = "",
  start_date = date_data_start, smooth_total = FALSE
) +
...
plot_growth(sgtf_by_age_posterior[region %in% "England"]) +
