...
- Data are all test-positive cases for England and by `r region_type`. We used `r data_type` data by `r date_type` date (figure \@ref(fig:plot-raw-data)). Models are based on data between `r min(obs$date)` and `r max(obs$date)`, which excludes the latest `r truncate_days` reported dates. Omicron is modelled from those cases reporting an S-gene target result (failure or not)^[`r if(open_data){"SGTF data from UKHSA: https://www.gov.uk/government/publications/covid-19-omicron-daily-overview"} else {"Data held confidentially"}`]. 
