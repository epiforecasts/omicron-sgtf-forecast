...
run_model <- FALSE
data_type <- "raw"
nhs_region_name <- NA # pass to load-data.R. Set to NA to return all regions
...
# get model result
regional_forecast_fits <- map(nhs_regions,
                              ~ safely_readRDS(here("transmission", "nhs_region", 
                                                    .x, "forecast-fits.rds"))) 
names(regional_forecast_fits) <- nhs_regions
regional_forecast_fits <- compact(regional_forecast_fits)

# get unnested
regional_forecasts <- map(nhs_regions,
                          ~ safely_readRDS(here("transmission", "nhs_region",
                                                .x, "forecasts.rds")))
names(regional_forecasts) <- nhs_regions
...
advantage <- regional_forecasts %>%
  as_tibble() %>%
  filter(nhs_region == "England" & value_type == "model" & 
...
```{r plot-voc-advantage, fig.cap="The transmission advantage of Omicron, modelled by NHS region in a fixed relationship to Delta (left) and a time-varying correlated relationship (right)."}
plot_voc <- filter(regional_forecasts, value_type == "voc_advantage") %>%
  ...
  geom_vline(xintercept = forecast_start, lty = 2, size = 1) +
    ...
    facet_wrap(facets = c("nhs_region", "variant_relationship"),
               scales = "freey_y", ncol = 2) +
    theme_classic() +
    theme(legend.position = "bottom", strip.background = element_blank())

ggsave(here("transmission", "figures", "growth.png"), plot_voc)
plot_voc
```

```{r plot-growth-rt, fig.cap="The growth rate of Omicron, modelled by NHS region in a fixed relationship to Delta (left) and a time-varying correlated relationship (right).", fig.dim=c(9,12)}
...
```{r plot-voc-frac, fig.cap="Fraction of cases attributable to Omicron, modelled by NHS region in a fixed relationship to Delta (left) and a time-varying correlated relationship (right)."}
plot_voc_frac <- filter(regional_forecasts, value_type == "voc_frac") %>%
  left_join(select(obs, date, nhs_region, share_voc), 
...
```{r plot-cases, fig.cap="Weekly cases shown on a log scale, modelled by NHS region in a fixed relationship to Delta (left) and a time-varying correlated relationship (right)."}
# Plot cases
...
loo_region <- imap_dfr(regional_forecast_fits,
  ...
  select(nhs_region, best_model, elpd_diff, se_diff)
```

We compared PSIS-LOO by region (table \@ref(tab:loo-table)). Using data for all England, comparing the models on PSIS-LOO indicated an estimated difference in expected log pointwise predictive density of `r loo_region %>% filter(nhs_region == "England") %>% pull(elpd_diff)` (with a standard error of `r loo_region %>% filter(nhs_region == "England") %>% pull(se_diff)`) for the `r loo_region %>% filter(nhs_region == "England") %>% pull(best_model)` model compared to the `r variant_relationships[!grepl(loo_region %>% filter(nhs_region == "England") %>% pull(best_model), variant_relationships)]` model. 
